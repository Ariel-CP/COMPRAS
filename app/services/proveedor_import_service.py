...
def _validate_email(email: Optional[str]) -> tuple[Optional[str], Optional[str]]:
...
def _validate_telefono(telefono: Optional[str]) -> tuple[Optional[str], Optional[str]]:
    """
...
def _validate_cuit(cuit: Optional[str]) -> tuple[Optional[str], Optional[str]]:
    """
...
def _parse_activo(value: Optional[str]) -> tuple[int, Optional[str]]:
    ...
    if value_clean in {"1", "si", "sí", "true", "yes", "activo", "enabled", "a"}:
        ...
        return 1, f"Valor 'Activo' inválido: {value} (use 1/Si/True o 0/No/False)"
    ...
    try:
        ...
        for fila_num, row in enumerate(reader, start=2):
            try:
                    ...
                    errores.append(
                        {
                            "fila": fila_num,
                            "codigo": "-",
                            "mensaje": "Código vacío o inválido",
                        }
                    )
                    rechazados += 1
                    continue
                ...
                if err:
                    errores.append({"fila": fila_num, "codigo": codigo, "mensaje": err})
                    rechazados += 1
        ...
        try:
            db.commit()
            logger.info(
                f"Importación completada: {insertados} insertados, {actualizados} actualizados, {rechazados} rechazados"
            )
        except Exception as ex:
            db.rollback()
